...
export default function ChoroplethLegend({
  ...
  return (
    <>
      {/* ── Desktop legend ── */}
      <div className="absolute bottom-4 left-4 z-10 hidden sm:flex flex-col gap-2 bg-parchment/95 backdrop-blur-sm px-4 py-3 rounded-2xl shadow-md">
        <div className="flex flex-col">
          <span className="font-heading text-[11px] font-semibold tracking-widest uppercase text-brown-dark">
            Recipe
          </span>
          <span className="font-heading text-[11px] font-semibold tracking-widest uppercase text-brown-dark -mt-0.5">
            Density
          </span>
          <span className="text-[10px] text-brown-medium mt-0.5 transition-all duration-400">
        ...
        <div className="flex flex-col gap-1.5">
          {desktopSteps.map((step) => (
            <div key={step.label} className="flex items-center gap-2">
              ...
              />
              <span className="text-[11px] text-brown-dark font-body tabular-nums leading-none">
                {step.label}
              </span>
            </div>
          ))}
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-[11px] text-brown-medium font-body leading-none">
              none
            </span>
          </div>
        </div>
      ...
      <div className="absolute bottom-14 left-3 z-10 flex sm:hidden flex-col gap-1 bg-parchment/95 backdrop-blur-sm px-2.5 py-2 rounded-xl shadow-md">
        {mobileSteps.map((step) => (
          <div key={step.label} className="flex items-center gap-1.5">
            <div
              ...
              style={{ backgroundColor: step.color }}
            />
            <span className="text-[10px] text-brown-dark font-body tabular-nums leading-none">
              {step.label}
            </span>
          </div>
        ))}
