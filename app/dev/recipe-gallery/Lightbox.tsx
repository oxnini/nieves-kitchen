...
/**
 * DEV-ONLY lightbox prototype for expanding an extra recipe image.
 *
 * Capture-phase Escape handler with stopPropagation so that, once integrated
 * into the recipe modal, the first Escape closes THIS overlay rather than the
 * recipe modal beneath it (mirrors the cook-mode Escape technique in
 * RecipeDetail).
 */
...
import type { DevRecipeImage } from './RecipeImageGallery';

export function Lightbox({ img, onClose }: { img: DevRecipeImage | null; onClose: () => void }) {
  useEffect(() => {
    if (!img) return;
