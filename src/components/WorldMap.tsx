...
export default function WorldMap({ recipes, onSelectRecipe }: WorldMapProps) {
  ...
  // Unique recipe coordinates for markers
  ...
  // Get fill color for a geography — choropleth style
  const getFill = useCallback(
    (geo: { properties: { name: string }; id?: string }) => {
      ...
      // If a region is selected, emphasize that region
      if (selectedRegion) {
      ...
      // Default world view: color by region density
      if (region) {
        const regionCount = recipesByRegion.get(region) || 0;
        return getChoroplethColor(regionCount, maxRegionCount);
      }
  ...
  function handleCountryClick(geo: { properties: { name: string }; id?: string }) {
    ...
    if (!selectedRegion && region) {
      ...
      // Second click (or click on country with recipes): select country
  ...
  // Regions that have recipes (for labels)
  const activeRegions = useMemo(() => {
    const regions: { region: CulinaryRegion; count: number; position: [number, number] }[] = [];
    for (const [region, count] of recipesByRegion.entries()) {
      if (count > 0) {
        regions.push({
          region,
          count,
          position: REGION_LABEL_POSITIONS[region],
        });
      }
    }
    return regions;
  ...
  const countryRecipes = selectedCountry
    ? recipesByCountry.get(selectedCountry) || []
    : [];
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-4 left-4 z-10 flex items-center gap-1 bg-white/90 backdrop-blur px-4 py-2 rounded-full shadow-md text-sm">
        <button
          ...
          className={`font-medium transition-colors ${
            !selectedRegion ? "text-terracotta" : "text-brown-medium hover:text-brown-dark"
          }`}
        >
          World
        </button>
        {selectedRegion && (
          <>
            ...
            <button
              onClick={goToRegion}
              className={`font-medium transition-colors ${
                !selectedCountry ? "text-terracotta" : "text-brown-medium hover:text-brown-dark"
              }`}
            >
              {selectedRegion}
            </button>
        ...
        >
          <ZoomableGroup
            ...
            filterZoomEvent={(e: Event) => {
              ...
              // Allow wheel events (includes pinch-as-wheel with ctrlKey on touch devices)
              // Block right-click and middle-click only
          ...
          >
            ...
            {/* Region labels with recipe counts (only at world zoom level) */}
            {!selectedRegion &&
              activeRegions.map(({ region, count, position: pos }) => (
            ...
            {/* Recipe markers (show when zoomed into a region) */}
            {selectedRegion &&
              markers
      ...
      <AnimatePresence>
        {selectedCountry && countryRecipes.length > 0 && (
          ...
          >
            <div className="p-4">
              <h3 className="font-heading text-lg font-bold text-brown-dark mb-1">
                {selectedCountry}
              </h3>
              ...
              <div className="space-y-3">
                {countryRecipes.map(recipe => (
                  ...
                  >
                    ...
                    <div className="p-3">
                      <h4 className="font-heading text-sm font-semibold text-brown-dark mb-1">
                        {recipe.name}
                      </h4>
                      <div className="flex gap-1.5 flex-wrap">
                        ...
                        {recipe.tags.slice(0, 2).map(tag => (
                          <span
                            key={tag}
                            className="text-[10px] px-1.5 py-0.5 rounded-full bg-parchment-dark text-brown-medium"
                          >
                            {tag}
                          </span>
                        ))}
