...
import {
  X, Clock, ChefHat, Users, Minus, Plus,
...
export default function RecipeModal({ recipe, onClose, isFavorited, onToggleFavorite }: RecipeModalProps) {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[100] flex items-start justify-center bg-black/50 backdrop-blur-sm overflow-y-auto p-4 sm:p-8"
        onClick={onClose}
      >
        <motion.div
          ...
          className="bg-parchment rounded-3xl shadow-2xl w-full max-w-2xl my-4 overflow-hidden"
        >
          {/* Hero image */}
          <div className="relative h-56 sm:h-72">
            ...
            <button
              ...
              className="absolute top-4 left-4 p-2 rounded-full bg-white/90 hover:bg-white transition-colors shadow"
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 p-2 rounded-full bg-white/90 hover:bg-white transition-colors shadow"
            >
          ...
          <div className="p-5 sm:p-8 space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {recipe.tags.map(tag => (
                <span
                  key={tag}
                  className="text-xs font-medium px-3 py-1 rounded-full bg-parchment-dark text-brown-medium"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="grid grid-cols-4 gap-2">
              {nutritionItems.map(n => (
                <div key={n.label} className="bg-white rounded-xl p-3 text-center shadow-sm">
                  ...
                  <div className="text-lg font-bold text-brown-dark">{n.value}<span className="text-xs font-normal">{n.unit}</span></div>
            ...
            {/* Flavor compass */}
            <div>
              <h3 className="font-heading text-lg font-semibold text-brown-dark mb-2">
                Flavor Profile
              </h3>
            ...
            <div>
              ...
              <div className="bg-white rounded-xl p-4 shadow-sm space-y-2">
                {recipe.ingredients.map((ing, i) => (
                  <div
                    key={i}
                    className="flex justify-between text-sm py-1 border-b border-parchment-dark last:border-0"
                  >
                    <span className="text-brown-dark">{ing.name}</span>
                    <span className="text-brown-medium font-medium">
            ...
            {/* Instructions */}
            <div>
              <h3 className="font-heading text-lg font-semibold text-brown-dark mb-3">
                Instructions
              </h3>
