...
Eastern Europe, and East Asia are anchor aesthetics** that predate or
sit outside this rule and stand as-is. All other regions follow the
cartouche + palette in the table below.
...
| **North America**        | Shared ink-impression grammar (redesigned 2026-05-24), with per-country cartouche silhouettes drawn from each country's folk-Americana vocabulary. **USA**: barn-quilt-derived octagon (8-sided with notched corners; inner double-rule with tin-punch dot pattern between the rules), sepia primary + barn-red accent. **Canada**: Hudson-Bay-blanket-stripe vertical rectangle (tall portrait format with two narrow horizontal stripe-bands top and bottom referencing point-blanket selvedge geometry, serrated postal-stamp outer edge), sepia primary + indigo secondary + butter-yellow tertiary (documented three-ink exception, preserved from the previous aesthetic). **Density layers (2026-05-24 enrichment):** every Section 10 stamp adds (a) four small ink-line corner glyph panels at the cardinal corners of the cartouche, vocabulary drawn from each country's regional folk-art heritage (USA: Pennsylvania-Dutch-hex-rosette for NE, Shaker round-box concentric-ring for NW, Mason-jar-lid concentric crimped rosette for SE, Diné/Navajo step-geometric block for SW; Canada: lobster-trap-lath silhouette grid for NE Atlantic, Haida-formline ovoid geometric for NW Pacific, fleur-de-lys folk variant for SE Québec, wheat-sheaf rosette for SW Prairies); (b) a thin fine-ink ecology horizon strip behind the central subject — heartland rolling wheatfield with a single grain silo and a cottonwood tree silhouette for USA; boreal lake at first light with a single canoe silhouette and a coniferous treeline along the far shore for Canada. Both layers render as flat ink line-art with transparent interior — the heritage vocabulary names the *shape*, never the material. Centerpieces preserved: diner coffee mug + pie wedge (USA), maple-syrup tap + bucket on a sugar-maple trunk (Canada). Reference: `united-states.webp`, `canada.webp`. | Barn red (`--stamp-ink-wine`) |
