...
> **Amendment (2026-07-05, steps 4-8 shipped on `revamp/phase-2-art`):** launched partial. Six of nine ink assets are in (`barley`, `dates`, `eggs`, `yoghurt`, `butter`, `olive-oil`); `honey`, `garlic`, `lamb` are still owed. `lib/pantry/landed.ts#landedPantryEntries()` fs-resolves which entries have art and is the single source of truth for both the `/pantry` shelf and the home `PantryTeaser`, so an entry surfaces automatically the moment its `<slug>.webp` lands — no code change needed to add the last three. The home page became a Server Component and `TableSpreadHero`/`LatestFromKitchen` now self-fetch via `useRecipes` (matching `CollectionsRow`) so the fs check can live at the page level. Verified: `tsc` + `build` clean, and a headless-Chrome pass drove both themes (shelf, overlay with/without prophetic block, zero-recipe line, cook-with-it link, backdrop/Escape close, teaser, FilterPanel chips, mobile tap targets) — 29/31 automated checks, the two non-passes being environmental console noise (Supabase 401 from headless Turnstile, external-host DNS), not app errors.

**Design:** `docs/superpowers/specs/2026-07-04-pantry-design.md` (parent: revamp spec §7).
**Picks (2026-07-04, all resolved — plan is unblocked):** card treatment = **variant D "etched"** (outline card, no filled surface); seal = **rosette** (confirmed against seven candidates); ﷺ = inline glyph. Execute steps in order.
