...
- [ ] **Cook-mode timer: recipe-aware analog dial (replaces preset row)**: *(Still valid, verified 2026-09-03. Note the file moved: it is `components/recipe/PageTimerStrip.tsx` + `PageTimerContext.tsx` now, not `PageTimer.tsx`.)* The current timer is a plain preset row + mm:ss countdown — functional but visually basic. Future direction: a circular dial driven by the recipe's own durations.
...
- [ ] **Rotate the Supabase service-role key** (security hygiene, not urgent): The Vercel env var `SUPABASE_SERVICE_ROLE_KEY` has a "Needs Attention" badge — Vercel suspects the key has been exposed somewhere (e.g. committed to a public repo, pasted in a screenshot, shared in a message). **What this key does**: it's an admin-level Supabase credential that bypasses all row-level-security rules. It's only used by the local `npm run seed:recipes` script, never in the browser, so the bug we just fixed wasn't caused by this. But if the key really is leaked, anyone who has it can read/write/delete any row in the database. **How to fix when ready**: (1) Supabase dashboard → Project Settings → API → "Reset service_role secret"; (2) update the value in Vercel → Settings → Environment Variables (or delete it there outright — nothing in the deployed app reads it, so removing it clears the badge permanently and shrinks the blast radius to the laptop); (3) update the value in your local `.env.local`; (4) confirm with `npm run recipes:check`; (5) the "Needs Attention" badge in Vercel should clear within a few minutes. *(Corrected 2026-09-03: step 3 used to say the key was needed for `npm run seed:mock`. That script is gone; the key is needed for `scripts/seed-recipes.ts`, which seeds the real catalogue.)*
