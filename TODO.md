...
- [ ] **WorldMap "Maximum update depth exceeded" warning**: The map now lives at `/atlas` (2026-07-03 pivot) — route context changed, bug unchanged; the fix is scheduled for the phase 1.5 seamlessness pass per the 2026-07-03 spec. React circuit-breaker fires during programmatic zoom (continent click, search-result selection, breadcrumb click). Suspected loop: `tick()` rAF → `setControlledPos` → `<ZoomableGroup>` re-render → `onMoveEnd` → `handleMoveEnd` snap-back → new `tick()`. Investigate the snap-back guard in `handleMoveEnd` (~line 480) and the `isAnimatingRef.current` flag (~line 421) in `components/WorldMap.tsx`. Possible fixes: earlier `if (isAnimatingRef.current) return` in `handleMoveEnd`, or skip `setControlledPos` on the final tick frame. Risk: zoom logic is load-bearing for all map navigation — needs careful manual testing of continent zoom, search zoom, and breadcrumb zoom in both themes after any change. Payoff: clean console, lower CPU/battery on programmatic zoom, removes a latent infinite-loop bug. Last seen: 2026-05-02 explore-page audit (item 3, the path from 19/20 → 20/20).
- [ ] **Phase 1.5 — Atlas seamlessness pass** (van Wijk camera via `d3.interpolateZoom`, rAF/`onMoveEnd` feedback-loop fix, gesture-cancels-animation, fade-not-pop layers, loading skeleton, reduced motion). Spec §9.
- [ ] **Collection shelf header on /recipes needs a design pass**: When a collection is active (`/recipes?collection=high-protein`), the shelf header band above the grid (title, description, X to leave — added 2026-07-03, commit `70d48db`) is functional but visually plain. Give it a proper editorial treatment during the next visual session — candidates: echo the home page's chapter-card language (accent bar per collection, Chapter numeral, recipe count), tighter integration with the controls row above it, better X affordance. File: `app/recipes/page.tsx` (the `activeCollection &&` block above the content section); collection data in `lib/collections.ts`. Keep the shelf mental model: it is a place you browse, not a filter — never re-add it to the Filters badge or chips.
