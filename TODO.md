...
- [~] **Recipe modal: no close affordance on Chrome mobile**: Two fixes shipped, one still needs a real-device pass.
  ...
  **Still open**: confirm on a real Chrome Android device. The backdrop strip is only ~66px tall (92dvh sheet), so it
  is a poor tap target even though it works; the swipe gesture is the intended escape now.
