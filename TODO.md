...
- [ ] **Cook-mode timer bell**: Phase 4 ships with a synthesised two-partial sine bell (in `hooks/usePageTimer.ts`, `playBell()`). It is warm-ish but not a real bell sample, **and the ring is too short** — needs to ring noticeably longer so it's hard to miss from across the kitchen (target ~3–4 seconds of audible decay instead of the current ~1.6s). Either extend the existing partials' decay envelopes or, better, source a CC0/royalty-free soft bell or wooden tone with a longer natural tail, encode as M4A (AAC) at ~96kbps mono, drop it at `public/sfx/timer-bell.m4a`, and swap `playBell` to load+play the asset. Keep the silent fallback for missing-file / autoplay-blocked cases.
- [ ] **Cook-mode timer panel should stay visible while you scroll**: `components/recipe/TimerPanel.tsx` currently scrolls away with the ingredients column. Make it `position: sticky; top: 96px` on `md+` standalone (clears the 64px navbar + breathing room) so it stays pinned while the user scrolls the right-column instructions; inside the modal, sticky against the modal scroll container with a small top inset. On mobile (single-column stack) leave it static so it doesn't fight the sticky step card at the bottom.
