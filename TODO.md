...
- [ ] **Atlas drill-down experiments — pick and bake in**: Three prototypes live behind `/atlas?exp=markers|bridge|reveal` (comma-separable) from the 2026-07-04 UX critique (score 24/40; full findings in that session). User reaction positive but no winner chosen yet. Next session: decide which to keep, strip the flag machinery from `WorldMapDesktop.tsx`, apply the winner(s) properly, and carry them to the mobile map (experiments are desktop-only today). Also still open from the critique: P3 stale-crumb on long manual pans (selection-driven crumb keeps the old name until the next click).
