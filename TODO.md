...
- [ ] **Atlas drill-down: carry the three shipped desktop behaviors to mobile**: All three 2026-07-04 critique prototypes were kept and baked into `WorldMapDesktop.tsx` (flags stripped): country markers at continent zoom everywhere, "See all N recipes" bridge pill bottom-right, continent click reveals the continent-scoped recipe panel immediately. Mobile (`WorldMapMobile` + `MobileMapCanvas`) does not have them yet — port all three, adapted to the bottom-sheet model. Also still open from the critique: P3 stale-crumb on long manual pans (selection-driven crumb keeps the old name until the next click).
