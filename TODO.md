...
- [ ] **Mobile navbar overflow / redesign**: On phones the top navbar (Nieves' Kitchen wordmark + page links) overflows horizontally — the info button gets cut off past the visible navbar edge, but the page is still horizontally scrollable so you can see the orphaned button sitting outside the bar. Two problems: (1) the page shouldn't scroll horizontally at all on mobile, (2) the navbar needs to actually fit the links it advertises. Likely fix is a floating mobile navbar (compact pill, possibly a menu/hamburger for overflow links, or a bottom tab bar a la Google/iOS native apps). Out of scope for the C+ mobile-map migration — pick up after M4 ships. File: `components/Navbar.tsx`.
